...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!req.query.n) {
    res
      .status(400)
      .json({ error: "Number of words (integer) not provided in url query." });
  } else if (
    typeof req.query.chars === "string" &&
    typeof req.query.n === "string"
  ) {
    res.status(200).json({
      words: getRandomWords(
        parseInt(req.query.n),
        getWordsWithCharsThatNeedWork(req.query.chars)
      ),
    });
...
function getRandomWords(
  ...
  if (listOfWords) {
    return Array.from(Array(numberOfWords).keys()).map(
      (_) => listOfWords[Math.floor(Math.random() * listOfWords.length)]!
    );
  }
...
function getWordsWithCharsThatNeedWork(charsThatNeedWork: string): string[] {
  ...
  return Array.from(
    words.filter((word) => {
      return charsThatNeedWork.split(",").some((char) => word.includes(char));
    })
  );
}
